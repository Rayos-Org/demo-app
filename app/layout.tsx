  ...
  return (
    <html lang="en" className={`${geistSans.variable} ${geistMono.variable}`}>
      <body className="min-h-screen flex flex-col bg-[#0a0a0a] text-white antialiased">
        {/* Testnet Banner — always visible, required for SCF reviewers */}
        <div className="bg-amber-500/10 border-b border-amber-500/20 text-amber-400 text-xs font-semibold text-center py-2 tracking-widest uppercase sticky top-0 z-50 backdrop-blur-sm">
          ⚠ Testnet Demo — No Real Funds Are Used ⚠
        </div>
        ...
        <header className="border-b border-white/8 bg-[#0a0a0a]/80 backdrop-blur-md sticky top-[33px] z-40">
          <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2 font-bold text-lg text-white hover:opacity-80 transition-opacity">
              ...
              <span>Rayos</span>
              <span className="text-xs text-gray-500 font-normal border border-white/10 rounded px-1.5 py-0.5 ml-1">demo</span>
            </Link>
        ...
        <footer className="border-t border-white/8 py-8 text-center text-xs text-gray-600">
          <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-2">
            <span>Built on Stellar Testnet with <span className="text-gray-400">@rayos/wallet-sdk</span></span>
            <a href="https://stellar.expert/explorer/testnet" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-300 transition-colors">
